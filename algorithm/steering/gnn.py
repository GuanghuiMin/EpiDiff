...
class mepognn(nn.Module):
    ...
    @torch.no_grad()
    def encode(self, x_node: torch.Tensor, SIR: torch.Tensor, od: torch.Tensor=None, max_od: float=None) -> torch.Tensor:
        """
        Return node-time embedding built from stcell heads (param_b, param_g).
        We concatenate them and average over time, so each node gets a vector of size 2*out_len.

        Args:
            x_node: (B, C_in, V, T_h)
            SIR   : (B, T_h, V, 3)  # kept for signature compatibility
            od    : required if glm_type == 'Dynamic' (see below)
        Returns:
            feat_vec: (B, V, 2*out_len)  # per-node embedding
        """
        if self.glm_type == 'Adaptive':
            # Build static supports from learned adaptive graph
            mob = torch.exp(torch.relu(self.g_rescaled * self.max_log))            # (V, V)
            g_fwd = mob / (mob.sum(1, keepdim=True) + 1e-12)
            g_bwd = mob.t() / (mob.t().sum(1, keepdim=True) + 1e-12)
            g_list = [g_fwd, g_bwd]

            # Run stcell to get mechanistic heads
            param_b, param_g = self.stcell(x_node, g_list)                         # (B, out_len, V, T_eff)

        elif self.glm_type == 'Dynamic':
            assert od is not None, "encode() needs 'od' for Dynamic mode."
            # od can be (B, T_h, V, V, 1) or already projected to (B, T_p, V, V, 1)
            if od.dim() == 5 and od.shape[1] != self.out_dim:
                # Simple projection: average over history (更稳健；避免einsum维度不匹配)
                mob = od.mean(dim=1).squeeze(-1)                                   # (B, V, V)
            elif od.dim() == 5 and od.shape[1] == self.out_dim:
                mob = od.squeeze(-1)                                               # (B, T_p, V, V) -> avg over future
                mob = mob.mean(dim=1)                                              # (B, V, V)
            elif od.dim() == 4:
                mob = od                                                           # (B, V, V)
            else:
                raise ValueError("Unexpected 'od' shape for Dynamic mode.")

            g = mob                                                                # (B, V, V)
            g_t = g.permute(0, 2, 1)                                              # (B, V, V)
            g_fwd = g / (g.sum(2, keepdim=True) + 1e-12)
            g_bwd = g_t / (g_t.sum(2, keepdim=True) + 1e-12)
            g_list = [g_fwd, g_bwd]

            param_b, param_g = self.stcell(x_node, g_list)                         # (B, out_len, V, T_eff)

        else:
            raise NotImplementedError("Invalid glm_type.")

        # Build a compact node embedding:
        # concat on channel -> (B, 2*out_len, V, T_eff), then avg over time -> (B, 2*out_len, V)
        feat = torch.cat([param_b, param_g], dim=1)
        feat = feat.mean(dim=-1)
        # transpose to (B, V, 2*out_len) so it's convenient for an MLP head
        feat_vec = feat.transpose(1, 2).contiguous()
        return feat_vec
