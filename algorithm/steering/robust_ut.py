...
def clamp_eta(eta, bounds_logR0_logTinf):
    ...
    return np.array(
        [
            np.clip(eta[0], lb_R0, ub_R0),
            np.clip(eta[1], lb_T, ub_T),
        ],
        dtype=float,
    )
...
def forward_incidence_given_init(k, x0_norm, N, theta):
    """Generate k-step incidence from a fixed initial normalized state."""
...
def forward_incidence_future(init_state, pop, h, theta):
    """Generate h-step future incidence from a given current state."""
    beta, gamma = map(float, theta)
    if h <= 0:
        return np.array([])
...
def map_estimate_eta_fixed_init(
    ...
    """MAP estimate of eta = (log R0, log Tinf) from historical incidence."""
    ...
    inits = [
        np.array(
            [
                0.5 * (bounds_logR0_logTinf[0][0] + bounds_logR0_logTinf[0][1]),
    ...
    if best is not None:
        eta_hat = clamp_eta(best.x, bounds_logR0_logTinf)
    else:
        eta_hat = np.array([np.log(2.0), np.log(5.0)], dtype=float)
...
def numerical_hessian(f, x, eps=5e-4):
    ...
    for i in range(d):
        for j in range(i + 1, d):
            ...
            H[i, j] = (f(xpp) - f(xpm) - f(xmp) + f(xmm)) / (4 * hi * hj)
            H[j, i] = H[i, j]

    return H


def laplace_cov_eta_fixed_init(
    y_hist,
    x0_norm,
    N,
    eta_hat,
    ...
    """Laplace covariance estimate in eta space."""
    def objective(eta):
...
def _fallback_from_hist_mean(y_hist, h, alpha_nb=5.0):
    ...
    # Negative-binomial predictive variance: mu + mu^2 / alpha.
...
def forecast_point_and_uncertainty_robust(
    ...
    *,
    ...
    """
    Robust autoregressive mechanistic forecast with Laplace-UT uncertainty.

    Important:
    - This function never receives or uses future ground-truth labels.
    - Multi-step rollout is autoregressive: after each one-step forecast,
      the model appends its own predicted incidence to the rolling history.
    - `horizon` only specifies how many future steps to generate.
    """
    ...
    assert T_p > 0, "horizon must be positive."
    ...
    for step in range(T_p):
        ...
        try:
            ...
            if mu_last_bar > 0:
                s = y_last_obs / max(1e-8, mu_last_bar)
            else:
                s = 1.0
            ...
            if bad:
                y_hat_step_arr, u_step_arr = _fallback_from_hist_mean(
                    y_hist_roll,
                    1,
                    alpha_nb=alpha_nb,
                )
                y_hat_step = float(y_hat_step_arr[0])
                u_step = float(u_step_arr[0])

                y_hat_all[step] = y_hat_step
                u_all[step] = u_step

                # Autoregressive rollout: append the model's own prediction.
            ...
            if (y_hat_step > max_inc_val) or (y_hat_step > max_ratio_vs_hist * hist_mean_base):
                y_hat_step_arr, _ = _fallback_from_hist_mean(
                    y_hist_roll,
                    1,
                    alpha_nb=alpha_nb,
                )
            ...
            # Autoregressive rollout even under numerical failure.
            y_hist_roll = np.concatenate([y_hist_roll, [y_hat_step]])
