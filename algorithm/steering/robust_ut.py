...
def forecast_point_and_uncertainty_robust(
    ...
    *,
    ...
    加速要点：
      - Σ 每 compute_sigma_every 步重算一次，中间步复用；
      - 幅度校准 s 只用均值参数（eta_hat）计算一次 μ_last；
      - L-BFGS 更快收敛的配置（滚动暖启动）。
    单步若检测到异常（NaN/Inf/负、爆炸或样本被筛空），立即用“历史均值兜底”对该步替换。
    """
    ...
    eta_prev = None  # 可选：如果你想要更“纯粹的”暖启动，这里也可以把 eta_prev 作为 inits 之一

    for step in range(T_p):
        # === 1) MAP（用较小 maxiter 和较宽松 ftol） ===
        eta_hat, pr_mu, pr_sd = map_estimate_eta_fixed_init(
            y_hist_roll, x0_hist, N_tr, alpha_nb=alpha_nb,
        ...
        if need_sigma:
            Sigma_eta = laplace_cov_eta_fixed_init(
                y_hist_roll, x0_hist, N_tr, eta_hat,
        ...
        # === 4) 幅度校准：只用均值参数（单次 ODE）===
        theta_mean = eta_to_theta(eta_hat)
        mu_tr_mean = forward_incidence_given_init(len(y_hist_roll), x0_hist, N_tr, theta_mean)
        mu_last_bar = float(mu_tr_mean[-1] if len(mu_tr_mean) > 0 else 0.0)
        y_last_obs  = float(y_hist_roll[-1]) if len(y_hist_roll)>0 else mu_last_bar
