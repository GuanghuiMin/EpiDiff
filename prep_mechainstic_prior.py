...
from algorithm.steering.robust_ut import forecast_point_and_uncertainty_robust as forecast_point_and_uncertainty
...
def run_all_windows(
    ...
    """
    Outputs:
      - y_hat_all:      (N_all, V, T_p)   point forecasts
      - uncert_all:     (N_all, V, T_p)   data-aware uncertainty
      - y_future_all:   (N_all, V, T_p)   future incidence from S differences
      - label_starts_all / _train / _test
      - meta:           dict with all key hyperparameters and notes
    """
    # ---- Load SIR ----
    ...
    # Valid windows: label_start - T_h >= 0 and label_start + T_p <= T
    # i.e. label_start in [T_h, T - T_p], inclusive. Hence use (T - T_p + 1) as the exclusive upper bound.
    first_all = T_h
    last_exclusive = T - T_p + 1  # include label_start == T - T_p
    ...
    assert (
        label_starts_all[-1] == T - T_p
    ), f"Window construction bug: last start {label_starts_all[-1]} != T - T_p {T - T_p}"

    if not (0 <= test_start_id <= T):
        raise ValueError("test_start_id out of range.")

    # Train/Test split by label_start
    ...
    print(
        f"Total ALL windows:   {N_all}  (from {label_starts_all[0]} to {label_starts_all[-1]})"
    )
    print(
        f"Total TRAIN windows: {N_tr}   (from {label_starts_train[0] if N_tr>0 else 'N/A'} to {label_starts_train[-1] if N_tr>0 else 'N/A'})"
    )
    print(
        f"Total TEST  windows: {N_te}   (from {label_starts_test[0]  if N_te>0 else 'N/A'} to {label_starts_test[-1]  if N_te>0 else 'N/A'})"
    )
    ...
    def process_one_window_node(label_start: int, v: int):
        ...
        sir_hist = np.stack(
            [S[hist_start:hist_end, v], I[hist_start:hist_end, v], R[hist_start:hist_end, v]],
            axis=-1,
        )
        ...
        try:
            y_hat, u = forecast_point_and_uncertainty(
                y_hist=y_hist,
                y_future=y_future,  # for data-aware uncertainty during evaluation
                sir_hist=sir_hist,
                alpha_nb=alpha_nb,
                ut_alpha=ut_alpha,
                ut_beta=ut_beta,
    ...
    meta = dict(
        ...
        V=int(V),
        note=(
            "y_hat_all/uncert_all/y_future_all cover all windows; "
            "label_starts_train/test are provided for downstream splits. "
            "Uncertainty is 'data-aware' since it uses y_future within the UT metric."
        ),
    )
    ...
    print(
        f"Shapes -> y_hat_all: {y_hat_all.shape}, "
        f"uncert_all: {uncert_all.shape}, y_future_all: {y_future_all.shape}"
    )


if __name__ == "__main__":
    # Edit these as needed
    SIR_PATH = "data/dataset/COVID-JP/jp20200401_20210921.npy"
    DATA_NAME = "COVID-JP"
    T_h = 14
    T_p = 14
    test_start_id = int(366 * 0.8)
    OUT_PATH = f"algorithm/uncert_out/{DATA_NAME}_uncert_th{T_h}_tp{T_p}.npz"
